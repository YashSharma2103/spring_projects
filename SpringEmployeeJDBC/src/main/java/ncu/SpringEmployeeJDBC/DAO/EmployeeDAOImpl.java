...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	public int updateEmployee(Employee e, int empId) {
		String sql = "update employee set employee_name=?, salary=?, address=? where employee_id=?;";
		...
		Employee emp = jdbc.query(query, new ResultSetExtractor<Employee>() {
			...
			public Employee extractData(ResultSet rs) throws SQLException, DataAccessException {
				if (rs.next()) {
					...
					e.setEmployee_salary(rs.getInt("salary"));
					e.setEmployee_address(rs.getString("address"));
	...
	public List<Employee> getEmployeeRecords() {
		String sql = "SELECT * FROM employee order by address;";
		List<Employee> el = jdbc.query(sql, new RowMapper<Employee>() {
			...
			public Employee mapRow(ResultSet rs, int n) throws SQLException {
				...
				emp.setEmployee_salary(rs.getInt("salary"));
				emp.setEmployee_address(rs.getString("address"));
	...
	public int[] saveBatch(List<Employee> empList) {
		String queryTruncate="truncate table employee_backup";
		jdbc.update(queryTruncate);
